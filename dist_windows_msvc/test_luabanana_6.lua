...
local function update_background()
    ...
    for row = 0, numrows - 1 do
        for col = 0, numcols - 1 do
            ...
            local i = (col + row) % 2
            banana_write_unichar_to(cp, bg[i], fg[i], background, x, y)
            cp = cp + 1
        end
    end
...
local function event_handler(event, ...)
    ...
    elseif event == BANANA_EVENT_MOUSE_MOVE then
        ...
        banana_write_utf8string_to(string.format("码点：%X", cp), nil, 0xffffffff, tooltip, 8, 8)
        -- 叠加
